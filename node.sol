...
contract NODERewardManagement {
    ...
    function createNode(address account, string memory nodeName, uint kind) external onlySentry {
        require(
    ...
    function _cashoutAllNodesReward(address account, bool takeTax)
    external onlySentry
    returns (uint256)
    {
        NodeEntity[] storage nodes = _nodesOfUser[account];
    ...
    function _cashoutAllNodesReward(address account, uint amount, bool takeTax)
    external onlySentry
    returns (uint256)
    {
        NodeEntity[] storage nodes = _nodesOfUser[account];
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            ...
            if(amount >= rewardsTotal + rewardNode) {
                ...
                _node.rewardAvailable = rewardsTotal.add(rewardNode).sub(amount);
                _node.lastClaimTime = block.timestamp;
                break;
            }
            rewardsTotal += rewardNode;
    ...
    function _cashoutAllNodesRewardKind(address account, uint kind, uint amount, bool takeTax)
    external onlySentry
    returns (uint256)
    {
        NodeEntity[] storage nodes = _nodesOfUser[account];
    ...
    returns (string memory)
    {
        ...
        for (uint256 i = 1; i < nodesCount; i++) {
            ...
            uint rewardAvailable = (block.timestamp.sub(_node.lastClaimTime)).mul(rewards[_node.kind]).div(86400).add(_node.rewardAvailable);

            _rewardsAvailable = string(
                abi.encodePacked(
                    _rewardsAvailable,
                    separator,
                    uint2str(rewardAvailable)
                )
            );
        }
