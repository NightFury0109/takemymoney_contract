...
interface INODERewardManagement {
    ...
    function _cashoutAllNodesRewardKind(address account, uint kind, uint amount, bool takeTax) external returns (uint256);
    ...
    function _cashoutAllNodesReward(address account, bool takeTax) external returns (uint256);
    function _cashoutAllNodesReward(address account, uint amount, bool takeTax) external returns (uint256);
...
contract TakeMyMoney is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    modifier onlyPresale() {
        require(presale == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
    ...
    function updateDevTreasury(address payable wall) external onlyOwner {
        treasury = wall;
    }
    ...
    function updateSellTax(uint256 _newTax) external onlyOwner {
        require(_newTax < 100 && _newTax >=0, "Invalid Tax");
        sellTax = _newTax;
    }
    ...
    function exchangeWithNode(uint kind, uint amount, string memory name, address account) external onlyPresale {
        for(uint i = 0; i < amount; i++) {
            nodeRewardManager.createNode(account, name, kind);
        }
    }
    ...
    function createNodeWithTokens(uint kind, string memory name) public {
        require(
        ...
        if (
            ...
            uint256 devTokens = contractTokenBalance.mul(devFee).div(100);
        ...
        nodeRewardManager.createNode(sender, name, kind);
    }
    ...
    function cashoutReward(uint256 blocktime) public {
        ...
        nodeRewardManager._cashoutNodeReward(sender, blocktime, true);
    }
    ...
    function getRewardAmountOf(address account, bool takeTax)
    public
    view
    onlyOwner
    returns (uint256)
    {
        return nodeRewardManager._getRewardAmountOf(account, takeTax);
    }

    function getRewardAmount(bool takeTax) public view returns (uint256) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        require(
            nodeRewardManager._isNodeOwner(_msgSender()),
            "NO NODE OWNER"
        );
        return nodeRewardManager._getRewardAmountOf(_msgSender(), takeTax);
    }
    ...
    function changeRewardPerNode(uint256[3] memory newReward) public onlyOwner {
        nodeRewardManager._changeRewardPerNode(newReward);
    }
    ...
    function changeClaimTax(uint256[3] memory newClaimTax) public onlyOwner {
        nodeRewardManager._changeClaimTax(newClaimTax);
    }
    ...
    function changeTaxChangeDays(uint256[3] memory newTaxChangeDays) public onlyOwner {
        nodeRewardManager._changeTaxChangeDays(newTaxChangeDays);
    }
